...
def cons_train(epoch, train_loader, model, optimizer, num_classes, noise_or_not, train_dataset):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        logits_conf = logits[sum_confident_ind]
        labels_conf = labels[sum_confident_ind]

        # conf loss
        conf_loss = F.cross_entropy(logits_conf, labels_conf, reduce=True)
        ...
        # training accuracy
        prec, _ = accuracy(logits_conf, labels_conf, topk=(1, 5))
...
file = open('./checkpoint/%s_%s' %
            (args.dataset, args.noise_type)+'_all_nomix.txt', "w")
